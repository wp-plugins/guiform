...
(function() {
	...
	$('.quick_edit').click(function(){
		...
		var data = {
					'action': 'guiform-form-quick-edit', 
					'id' : id
				};
	...
	$('.wp-list-table').on('click', '.button-primary.save', function(){
		...
		var field = $("input[class=fields]:checked").map(function () {return this.value;}).get()
		var db = $('select[name="db_con"]').val();
		$('.spinner').show();
		
		var data = {
					'action': 'guiform-form-quick-edit-save', 
					'id' : id,
					'db'   : db,
					'title' : title,
					'field' : field
