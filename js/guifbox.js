...
(function (b, c) {
  ...
  b.widget("guiform.guifbox", b.extend({}, a, {
    _create: function () {
      ...
      this._build();
      b(document)
        .on("click", "#guifbox-fullscreen", function () {
          if(b("#guifbox-window")
            .hasClass("guifbox-maxscreen")) {
            b("#guifbox-window")
              .removeClass("guifbox-maxscreen");
            b("body")
              .css("overflow", "");
            b("#guifbox-body")
              .css("height", "auto");
      ...
      b(document)
        .on("click", "#guifbox-save", function () {
          b("#guifbox-spinner")
            .show();
          e.save();
          b("#guifbox-window")
    ...
    _build: function () {
      ...
      (b("#guifbox")
        .size() > 0) ? b("#guifbox-window, #guifbox-overlay")
        .show() : b("body")
        .append(d);
      (this.options.status != "loading") ? b("#guifbox-header")
        .append("<span data-icon='&#xe082;' id='guifbox-close'></span>") : b("#guifbox-close")
        .remove();
      (this.options.fullScreenButton == true) ? b("#guifbox-header")
        .append("<span id='guifbox-fullscreen' title='Full Screen'></span>") : b("#guifbox-fullscreen")
        .remove();
      (this.options.status == "loading" || this.options.status == "confirm") ? b("#guifbox-lockscreen")
        .show() : b("#guifbox-lockscreen")
        .hide();
      b("#guifbox-overlay")
        .css({
      ...
      b("#guifbox-title")
        .html(this.options.title.toUpperCase());
      b("#guifbox-body")
      ...
      if(this.options.saveButton == true) {
        b("#guifbox-button")
          .prepend("<span id='guifbox-spinner'></span>");
        b(".button-primary")
          .attr("id", "guifbox-save")
          .text("Save")
      }
      if(this.options.closeButton == true) {
        b("#guifbox-button")
          .prepend("<a id='guifbox-close' class='button-secondary' href='javascript:void(0);' onclick='return false;'>Close</a>")
      }
      ...
      if(this.options.overlay == false) {
        b("#guifbox-overlay, #guifbox-lockscreen")
          .remove()
      }
      this._createHTML()
    },
    ...
    _event: function () {
      ...
      b("#guifbox-overlay, #guifbox-close, #guifbox-cancel")
        .click(function (f) {
          e._close()
        });
      b(document)
    ...
    _close: function () {
      ...
      if(d.status == "success" && d.reload == true) {
        ...
        b("#guifbox-title")
          .html("Loading");
        b("#guifbox-button, #guifbox-close")
          .remove();
        b("#guifbox-body")
          .html("<p>Page Loading</p>");
        d.reload = true;
        d.status = "loading";
        b("#guifbox")
          .remove();
    ...
    _confirm: function (d) {
      ...
      b.ajax({
        ...
        beforeSend: function () {
          ...
          b("#guifbox-body")
            .css("margin-bottom", "8px")
            .html("<p>Please wait while the server processing your request.</p>");
          var g = (b("body")
            .height() - b("#guifbox-window")
            .height()) / 2;
          var f = (b("body")
