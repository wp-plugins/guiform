...
(function (a, b) {
    a.widget("GuiForm.form", {
        ...
        _create: function () {
            var c = this.element;
            var d = this;
            this.options.submitText = a('button[type="submit"]', c).text() || "";
            ...
            a('input[type="reset"]', this.element).click(function (e) {
                d.reset()
            })
        },
            ...
            a.ajax({
                ...
                success: function (j) {
                    if (a(".gui-file-bar.upload").size() < 1) {
                        a('button[type="submit"]', f).text(c.submitText).removeClass("disabled")
                    }
                }
            })
        },
        reset: function () {
            ...
            a('button[type="submit"]', c).text(this.options.submitText).removeClass("disabled");
            xhr = {};
            xhr_name = {};
            xhr_state = {};
            xhrData = {}
        ...
        FileInit: function () {
            ...
            a('.guif-dropzone input[type="file"]').on("change", function (j) {
                ...
                a('button[type="submit"]', d).text("Loading").addClass("disabled");
                a.each(i, function () {
                    var r = this.name;
                    ...
                    var o = '<div id="file-' + m + '" class="gui-file selected" title="' + u + '">												<div class="gui-file-bar upload"></div>												<div class="gui-file-wrap clearfix">													<span class="gui-file-name">' + r + "</span>													" + o + '													<span class="gui-file-remove cancel icon-fontawesome-webfont-96" title="cancel"></span>												</div>											</div>';
                    ...
                    if (t == 0) {
                        ...
                        xhr[m].onreadystatechange = function (v) {
                            if (xhr[m].readyState == 4) {
                                ...
                                if (a(".gui-file-bar.upload").size() < 1) {
                                    a('button[type="submit"]', d).text(c.submitText).removeClass("disabled")
                                }
                            }
                        };
                        var n = new FormData();
        ...
        submit: function (g) {
            ...
            var d = this.options;
            var c = a(f);
            a('button[type="submit"]', f).text("Loading").addClass("disabled");
            ...
            e.load(function (n) {
                ...
                a('button[type="submit"]', f).text(d.submitText).removeClass("disabled")
            })
        },
        getDoc: function (e) {
            var d = null;
