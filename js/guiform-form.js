...
(function (a, b) {
  a.widget("GuiForm.form", {
    ...
    _create: function () {
      var c = this.element;
      var d = this;
      this.options.submitText = a('button[type="submit"]', c)
        .html() || "Submit";
      this.options.loadText = a('button[type="submit"]', c)
        .attr("data-loader") || "Loading";
      ...
      if(window.FormData !== b && window.File && window.FileList) {
        ...
        a('input[type="file"][multiple="multiple"]')
          .on("change", function (g) {
            ...
            c.append('<input type="hidden" class="guiform-file-' + h + '" name="guiform-item" value="' + a(this)
              .parents(".item")
              .attr("id") + '">');
            ...
            a(".gui-files", f)
              .prepend(e);
            a(".gui-file.selected", c)
              .addClass("restored-item")
              .animate({
                opacity: 1
              }, 1300, function () {
                a(this)
                  .removeClass("restored-item, selected");
                d.tooltip(".gui-file")
              });
      ...
      a(document)
        .on("click", ".gui-file-remove", function () {
          var f = a(this)
            .parent()
            .parent();
          var e = f.parent();
          f.addClass("removed-item")
      ...
      a(document)
        .on("keydown", ".ui-spinner-input", function (f) {
      ...
      a(document)
        .on("click", ".gui-file-remove", function (f) {
          var g = a(this)
            .parents(".gui-file")
            .attr("id")
            .split("-")[1];
      ...
      a('button[type="reset"]', this.element)
        .click(function (e) {
          d.reset()
        });
      a(window)
        .load(function () {
          a('button[type="submit"]', this.element)
            .removeAttr("disabled")
        });
      a(".ui-button-text")
        .each(function () {
          var e = a("[data-icon]", this)
            .attr("title");
          a("[data-icon]", this)
            .removeAttr("title");
          a(this)
    ...
    FileRemove: function (e, i) {
      ...
      if(a(".gui-file-bar.upload")
        .size() < 1) {
        a('button[type="submit"]', parent)
          .html(c.submitText)
    ...
    FileInit: function () {
      ...
      a('.guif-dropzone input[type="file"]')
        .on("change", function (j) {
          var i = a(this)
            .prop("files");
          ...
          var f = a(this)
            ...
            .replace(/\s/g, "")
            .split(",");
          var k = a.bytesSize(a(this)
            .attr("file-maxsize"));
          a('button[type="submit"]', d)
            .html(c.loadText)
            .addClass("disabled");
          a.each(i, function () {
            var r = this.name;
            var m = Math.floor(Math.random() * 90000);
            var s = r.substr((~-this.name.lastIndexOf(".") >>> 0) + 2)
              .toLowerCase();
            var q = this.size;
            var t = 0;
            var u = "";
            var l = guif.today + "-" + Math.random()
              .toString(36)
              .slice(3) + "." + s;
            if(a.inArray(s, f) <= -1) {
              ...
              xhr[m].onreadystatechange = function (v) {
                if(xhr[m].readyState == 4) {
                  ...
                  if(data.status == "error") {
                    a("#file-" + data.error.id)
                      .attr("title", data.error.message);
                    a("#file-" + data.error.id)
                      .find(".gui-file-bar")
                      .remove();
                    a("#file-" + data.error.id)
                    ...
                    a("#file-" + data.info.id)
                      .find(".gui-file-size")
                      .html(a.bytesToSize(data.info.size));
                    a("#file-" + data.info.id)
                      .find(".gui-file-remove")
                      .removeClass("cancel")
                      .attr("title", "remove");
                    a(".guiform-file-" + data.info.id)
                      .remove();
                    xhrData[m] = data.info
                  } if(a(".gui-file-bar.upload")
                    .size() < 1) {
    ...
    remove_error: function () {
      var c = this.element;
      a(".error, .error-message", c)
        .each(function () {
          (a(this)
            .hasClass("error-message")) ? a(this)
            .remove() : a(this)
            .removeClass("error")
        })
    },
    ...
    submit: function (g) {
      ...
      a('button[type="submit"]', f)
        .html(d.loadText)
        .addClass("disabled");
      var h = "unique-" + (new Date()
        .getTime());
      var e = a('<iframe style="width: 100%; height : 200px;" src="javascript:false;" name="' + h + '" />');
      e.hide();
      if(Object.keys(xhrData)
        .length && a("input[class^='guiform-file-']")
        .size() == 0) {
      ...
      e.load(function (n) {
        ...
        if(l.indexOf("Fatal error") !== -1) {
          ...
          if(a("input[class^='guiform-file-']")
            .size() > 0) {
            l = a.parseJSON(l);
            if(l.status == "error") {
              a("#file-" + l.error.id)
                .attr("title", l.error.message);
              a("#file-" + l.error.id)
                .find(".gui-file-name")
                .html(l.error.file);
              a("#file-" + l.error.id)
              ...
              a("#file-" + l.info.id)
                .find(".gui-file-size")
                .html(a.bytesToSize(l.info.size));
              a("#file-" + l.info.id)
                .find(".gui-file-remove")
                .removeClass("cancel")
                .attr("title", "remove");
              a(".guiform-file-" + l.info.id)
                .remove();
            ...
            if(l.status == "fail") {
              ...
              if(l.status == "error") {
                ...
                if(l.thank_you.checked == "defualt") {
                  a("body")
                    .css("padding", "30px")
                    .html('<p style="text-align: center;"><span style="font-size: 16pt;"><strong>Thank You!</strong></span></p> <p style="text-align: center;">We will get in touch with you shortly.</p>')
                } else {
