    ...
    a.widget("GuiForm.form", {
        ...
        _create: function () {
            ...
            a("label").click(function () {
                a(this).parents("span").find("input").trigger("click")
            })
        },
        ...
        FileInit: function () {
            ...
            a('.guif-dropzone input[type="file"]').on("change", function (j) {
                ...
                a.each(i, function () {
                    ...
                    var v = "";
                    var u = new Date();
                    var l = u.getDate() + "-" + Math.random().toString(36).slice(3) + "." + s;
                    if (a.inArray(s, f) <= -1) {
                        o = '<span class="gui-file-error">Failed</span>';
                        v += "Upload FAILED, invalid file type !";
                        t++
                    } else {
                        if (q > k) {
                            o = '<span class="gui-file-error">Failed</span>';
                            v += "Upload FAILED, file is too large !";
                            t++
                        } else {
                            o = '<span class="gui-file-size"><img src="' + guif.images + 'fb-loader.gif" /></span>										'
                        }
                    }
                    var o = '<div id="file-' + m + '" class="gui-file selected" title="' + v + '">												<div class="gui-file-bar"></div>												<div class="gui-file-wrap clearfix">													<span class="gui-file-name">' + r + "</span>													" + o + '													<span class="gui-file-remove cancel icon-fontawesome-webfont-96" title="cancel"></span>												</div>											</div>';
                    ...
                    if (t == 0) {
                        ...
                        xhr[m].upload.onprogress = function (x) {
                            var w = parseInt(x.loaded / x.total * 100);
                            p.css("width", w + "%")
                        };
                        xhr[m].onreadystatechange = function (w) {
