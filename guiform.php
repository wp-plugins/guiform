...
class GuiForm extends GuiForm_API{
	...
	public function ajax_form_preview(){
		global $guif;
		$id = esc_sql($_GET['id']);
		$height = esc_sql($_GET['height']) - 100;
		echo '<div style="background-color: rgb(240, 240, 240); text-align: right;">
						<label style="float: left; padding: 5px; text-align: left; width: 60%;">URL : <input type="text" value="'. $guif->permalink($id) .'" style="width: 85%; background-color: #FFFFFF;" readonly="readonly"></label>
						<a style="padding: 10px; display: inline-block; font-weight: bold;" target="_blank" href="'. $guif->permalink($id) .'">Full Preview</a>
					</div>';
	...
	public function help_form_builder(){
		...
		$screen->add_help_tab( array(
			...
			'content' => "<p><strong>Save Form</strong> - Save or update form.</p>
										...
										<p><strong>Styles</strong> - Display form styling options such as colors fonts and costume CSS.</p>
										...
										<p><strong>Form Properties</strong> - Display field properties.</p>
	...
	public function create_table(){
		...
		if(empty($version) || $version != GUIFORM_VERSION){
			...
			$options = array(array('settings', 'license_key', 'BETA TEST'),
											 ...
											 array('settings', 'autosave_time', 180),
...
if(!isset($guif)){
	...
	// Load AJAX class
	$foobar = new GuiForm_Ajax;  // correct
	$foobar->init();
}