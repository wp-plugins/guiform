...
Description: SIMPLE - FAST - EFFICIENT Creating Forms on your website has never been easier with GuiForm.
...
class GuiForm extends GuiForm_API{
	...
	public function update_plugin(){
		...
		if (!is_plugin_active('guiform/guiform.php')) {
			...
			$wpdb->query("ALTER TABLE $wpdb->guiform ADD `save_entries` BOOLEAN NOT NULL DEFAULT TRUE AFTER `status`");
	...
	public function ajax_form_preview(){
		...
		echo '<table style="background-color: #00ABEB; font-weight: bold; width: 100%; padding: 5px 10px 0;">
						<tr>
							<td style="width: 50%;"><div>Link :</div><input type="text" value="'. $guif->permalink($id, '', false) .'" style="width: 95%; background-color: #FFFFFF;" readonly="readonly"></td>
		...
		echo "<iframe style='width: 100%; padding: 20px 0px; height: ".$height."px' src='". $guif->permalink($id, '', false) ."'></iframe>";
		die();
	}
  ...
  /**
	...
	*/
	public function help_form_builder(){
		...
		$screen->add_help_tab( array(
		  'id' => 'guiform-help-tab-pass-parameter',
		  'title' => 'Pass Parameter Value',
			'content' => "<p>Enable to pass the value from another form or using URL query string to populate the text and default value of your form.</p>
										<ul><li><p><strong>{user_id}</strong> - User ID.</p></li>
											<li><p><strong>{user_username}</strong> - User username.</p></li>
											<li><p><strong>{user_email}</strong> - User email address.</p></li>
											<li><p><strong>{user_firstname}</strong> - User first name.</p></li>
											<li><p><strong>{user_lastname}</strong> - User last name.</p></li>
											<li><p><strong>{user_display_name}</strong> - User display name.</p></li>
											...
											<li>
												<p><strong>{param_[key]}</strong> - Retrieve value from POST and GET form method where key is the key name of the variable.</p>
												<p><code>Example: htt://yourdomain.com/form/?username=$user->user_login&email=$user->user_email</code></p>
												<p><strong>{param_username}</strong> - display <code>$user->user_login</code></p>
												<p><strong>{param_email}</strong> - display <code>$user->user_email</code></p>
											</li>
											<li>
	...
	public function admin_scripts($hook_suffix){
	    ...
	    //Load all wordpress jquery necessary files
			...
			wp_enqueue_script('guifbox', plugins_url('guiform/js/guifbox.js'), $js, GUIFORM_VERSION, true);
		  ...
		  wp_enqueue_script('caret', plugins_url('guiform/js/jquery.caret.min.js'), false, null, true);
			...
			wp_enqueue_script('guiform-builder', plugins_url('guiform/js/guiform.js'),false, GUIFORM_VERSION, true);
	    ...
	    foreach($fonts as $value){
			...
			wp_localize_script('guiform-builder', 'guiBuilder', array(
			  ...
			  'emails'      => $guif->get_mail(),
				...
				'preview_url' => $this->permalink('', '', false),
	...
	public function create_table(){
		...
		$create_table['guiform'] = "CREATE TABLE IF NOT EXISTS `$wpdb->guiform` (
									...
									`save_entries` TINYINT(1) NOT NULL DEFAULT TRUE,
	...
	public function footer_script() {
		...
		wp_register_script('guiform-core', includes_url('js/jquery/ui/jquery.ui.core.min.js'));
		wp_register_script('guiform-widget', includes_url('js/jquery/ui/jquery.ui.widget.min.js'));
		wp_register_script('guiform-spinner', includes_url('js/jquery/ui/jquery.ui.spinner.min.js'));
		wp_register_script('guiform-tooltip', includes_url('js/jquery/ui/jquery.ui.tooltip.min.js'));
		wp_register_script('guiform-button', includes_url('js/jquery/ui/jquery.ui.button.min.js'));
		wp_register_script('guiform-touch-punch', includes_url('js/jquery/jquery.ui.touch-punch.js'));
		
		wp_register_script('guiform-maskedinput', plugins_url('guiform/js/jquery.maskedinput.min.js'));						
		wp_register_script('guiform-form', plugins_url('guiform/js/guiform-form.js'), false, GUIFORM_VERSION);
		
		$this->wp_localize_script();
		
		wp_print_scripts('guiform-jquery');
		wp_print_scripts('guiform-migrate');
		wp_print_scripts('guiform-core');
		wp_print_scripts('guiform-widget');
		wp_print_scripts('guiform-spinner');
		wp_print_scripts('guiform-tooltip');
		wp_print_scripts('guiform-tooltip');
		wp_print_scripts('guiform-touch-punch');
		wp_print_scripts('guiform-button');
		
		wp_print_scripts('guiform-maskedinput');
    ...
    wp_deregister_script('jquery');
		wp_register_script('guiform-jquery', includes_url('js/jquery/jquery.js'));
		wp_register_script('guiform-migrate', includes_url('js/jquery/jquery-migrate.min.js'));
		wp_register_script('guiform-ui', $url, false, null);		
		wp_register_script('guiform-maskedinput', plugins_url('guiform/js/jquery.maskedinput.min.js'));						
		wp_register_script('guiform-form', plugins_url('guiform/js/guiform-form.js'), false, GUIFORM_VERSION);
	...
	private function wp_localize_script(){
			wp_localize_script('guiform-form', 'guif', array(
			  ...
			  'form_id'  => $this->id,
			  'user'     => $this->userdata(),
			  'current'  => $this->current_param(),
			  'param'    => isset($_REQUEST['param']) ? array_map("stripslashes", $_REQUEST['param']) : array_map("stripslashes", $_REQUEST)
			));
	}
	
	private function current_param(){
