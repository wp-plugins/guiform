...
class GuiForm_Module_Admin extends GuiForm_Module {
	...
	public function menu(){
	  ...
	  $this->_adminPage['uninstall'] = add_submenu_page(GuiForm_Plugin::NAME, __('Uninstall', GuiForm_Plugin::NAME), __('Uninstall', GuiForm_Plugin::NAME), 'manage_guiform', 'guiform-uninstall', array(&$this, 'adminPage'));
		...
		$this->_addAction('load-'. $this->_adminPage['builder'],  array('GuiForm_Render_Builder', 'help'));

	  $class_list = array('forms', 'entry', 'export', 'builder', 'settings', 'uninstall');
	...
	public function classListTable(){
		...
		else if($screen->id == 'guiform_page_guiform-builder'){
			$this->_render = new GuiForm_Render_Builder($screen);
		}
		else if($screen->id == 'guiform_page_guiform-entry'){
		...
		else if($screen->id == 'guiform_page_guiform-backup'){
			$this->_render = new GuiForm_Render_Backup($screen);
		}
	...
	public function actionLink($links, $file){
		if ( $file == plugin_basename( GUIFORM_BASEFILE ) ) {
			array_unshift(
				...
				sprintf(
					'<a href="%s">%s</a>',
					admin_url( 'admin.php?page='. GuiForm_Plugin::NAME .'/form-builder' ),
					esc_html__( 'Create Form', GuiForm_Plugin::NAME )
				),
				sprintf(
					'<a href="%s">%s</a>',
					admin_url( 'admin.php?page='. GuiForm_Plugin::NAME .'/settings' ),
					esc_html__( 'Settings', GuiForm_Plugin::NAME )
				)
			);
		}
	...
	public function pluginLinks($links, $file){
		if($file == plugin_basename(GUIFORM_BASEFILE)){
			$links[] = '<a href="https://www.guiform.com/">'. __('Documentation', GuiForm_Plugin::NAME) .'</a>';
			$links[] = '<a href="http://wordpress.org/support/plugin/guiform">'. __('Support', GuiForm_Plugin::NAME) .'</a>';
		}
