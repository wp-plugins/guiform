...
class GuiForm_Module_Post extends GuiForm_Module {
  ...
  private $_sql = '';
    ...
    header("Pragma: no-cache");
		...
		$this->_addAdminAction(GuiForm_Plugin::ACTION_SAVE_ENTRY, 'saveEntry', true, true);
	...
	public function saveForm(){
  	...
  	$entry_fields = array('id' => array('ID', true), 
		...
		foreach($post['data'] as $item){
			if($item['type'] != "f_image_captcha" && $item['type'] != "f_confirm_password" ){
				$entry_fields[$item['name']] = array($item['label'], false);
			}
		}
  	...
  	if($this->_id > 0){
    	...
    	$wpdb->insert($wpdb->guiform, 
  		...
  		$confirmation = array('type' => 'default', 'url' => '', 'custom' => '');
  ...
  private function _checkError(){
	...
	public function exportEntryCsv(){
  	...
  	$form_field = array('id' => ' ID', //Do not remove the space before th ID word. 
	...
	public function exportFormSql($table = array(), $zip = false){
	  ...
	  $form_column = $wpdb->get_col("SHOW COLUMNS FROM $wpdb->guiform");
		...
		$return .= "-- MYSQL Version: ". mysqli_get_server_info() ." \n";
		...
		foreach($forms as $id) {
		  ...
		  $return .= "-- \n";
			...
			if(sizeof($result) && isset($_POST['entry'])){
...
//			gzclose($zp);
	...
	public function saveEntry(){
  	...
  	if(isset($_POST['guiform-upload'])){
  		...
  		$guif_fields = $guiform->form($this->_id, 'data');
	  	...
	  	foreach($formFields as $key => $props){
	  		...
	  		if($required && $props['type'] != 'f_file' && $_POST[$this->field] != "0" && (!isset($_POST[$this->field]) || empty($_POST[$this->field]))){
		        ...
		        $this->error[$this->item] = __('Invalid link.', GuiForm_Plugin::NAME);
	  			}
				}
				...
				if($props['type'] == 'f_image_captcha' || $props['type'] == 'f_confirm_password'){
				...
				foreach($settings as $key => $value){
					...
					if($type == 'confirmation' || ($type == 'notification' && $data['status'])){
						...
						if($type == 'notification'){
							...
							$response[$index]["subject"] = str_replace("{ip_address}", $guiform->ip, $response[$index]['subject']);
							$response[$index]["subject"] = str_replace("{form_id}", $this->_id, $response[$index]["subject"]);
							$response[$index]["subject"] = str_replace("{form_title}", $title, $response[$index]["subject"]);
							$response[$index]["subject"] = str_replace("{id}", $insert_id, $response[$index]["subject"]);
						}
						
						$response[$index][$message] = stripcslashes($data[$message]);
						$response[$index][$message] = str_replace("{ip_address}", $guiform->ip, $response[$index][$message]);
						$response[$index][$message] = str_replace("{form_id}", $this->_id, $response[$index][$message]);
						$response[$index][$message] = str_replace("{form_title}", $title, $response[$index][$message]);
							...
							foreach($matches[1] as $key){
								if($guiform->isSerial($fields[$key])){
									...
									foreach(unserialize($fields[$key]) as $row => $val){
										if(is_array($val)){
											$file = $guiform->upload_folder($row, $val['path'], true);
  ...
  public function validText($type = "", $value = array()){
  	if(!empty($this->value)){
  		if($type == "email"){
  			...
  			$this->error[$this->item] = __('Your input is invalid for this field.', GuiForm_Plugin::NAME);
  		}
  ...
  public function uniqueValue(){ 
  	global $wpdb;
  	if(count($wpdb->get_row($wpdb->prepare("SELECT $this->field FROM $wpdb->guiform_form$this->_id WHERE $this->field = '%s'", $this->value), ARRAY_N)) > 0){
  		$this->error[$this->item] = $this->value .' '. __("is already exist.", GuiForm_Plugin::NAME);
  	}
  }
