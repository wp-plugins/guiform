...
class GuiForm_Module_Api{
	...
	public function assets($path = '', $abspath = false){
		return self::_resource('assets', $path, $abspath);
	}
	...
	private function _resource($type, $path = '', $abspath = false){
		if($type == 'wp-includes'){
			return includes_url($path);
		}
		else{
			if(empty($path) || $path == false)
				return ($abspath) ? GUIFORM_ABSPATH . $type .'/' : plugins_url(GuiForm_Plugin::NAME .'/'. $type .'/');
			else
				return ($abspath) ? GUIFORM_ABSPATH . $type .'/'. $path : plugins_url(GuiForm_Plugin::NAME .'/'. $type .'/'. $path);
		}
	}
  ...
  /**
	...
	public function updateOption($name, $data, $type = false, $id = false){
		...
		if($type == 'settings' || $type == 'confirmation' || $type == 'smtp' || $type == 'mailchimp'){
		...
		else if($type == 'confirmation' || $type == 'settings' || $type == 'smtp' || $type == 'mailchimp'){
			$wpdb->update($wpdb->guiform_options, 
									array('type' => $type,
												'name' => $name,
												'value' => $data),
  ...
  public function printForm($id, $atts = array()){
  	$atts['id'] = $id;
