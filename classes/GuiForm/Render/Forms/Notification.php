	...
	*/
	...
	public function scripts($hook_suffix){
		...
		if($hook_suffix == 'toplevel_page_'. GuiForm_Plugin::NAME && $_REQUEST['tab'] == 'notification'){
			...
			wp_enqueue_style('jquery-ui-theme', $guiform->vendor('jquery-ui/css/custom-theme/jquery-ui-1.9.2.custom.min.css'), false, GuiForm_Plugin::VERSION, 'all');
	...
	public function toHTML() {
		?>
		<div id="guiform-manager" class="wrap form-notification metabox-holder">
		...
		?>
			...
			<form method="POST">
				...
				<table class="table-settings" width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td rowspan="9" style="width: 200px; vertical-align: top; background: none repeat scroll 0 0 #F1F1F1;">
					...
					<tr>
						...
						<td class="column-left" colspan="2">
						  <select name="from" class="<?php if(!empty($this->errors['from'])) echo 'error'; ?>">
								<option value=""></option>
								<?php
									...
									foreach($guiform->form($this->form, 'data') as $mail){
										if($mail['type'] == 'f_email' || (isset($mail['validation']['text']) && $mail['validation']['text'] == 'email')){
											$email = '{'.$mail['name'].'}';
						...
						<td class="column-attachment" rowspan="4">
							<div id="attachment-item">
								<p style="text-align: center; font-size: 21px; margin-top: 35px;">
									Upgrade to <a href="https://www.guiform.com/" style="color: rgb(40, 130, 214); text-decoration: none;">GuiForm Pro</a> to enable file attachment.
								</p>							
							</div>
						</td>
					</tr>
					<tr>
						...
						<td class="column-left" colspan="2">
							<input class="<?php if(!empty($this->errors['to'])) echo 'error'; ?>" type="text" value="<?php echo $this->var['to']; ?>" name="to">
						</td>
					</tr>
					<tr class="<?php if(empty($this->var['cc'])) echo 'hide'; ?>"> 
						<td class="border-left col-remove">
							<span class="remove" id="button-cc" data-icon="&#xe093;"></span>
						</td>
						<td class="column-right" colspan="1">Cc</td>
						<td class="column-left">
							<input class="<?php if(!empty($this->errors['cc'])) echo 'error'; ?>" type="text" value="<?php echo $this->var['cc']; ?>" name="cc">
						</td>
					</tr>
					<tr class="<?php if(empty($this->var['bcc'])) echo 'hide'; ?>"> 
						<td class="border-left col-remove">
							<span class="remove" id="button-bcc" data-icon="&#xe093;"></span>
						</td>
						<td class="column-right" colspan="1">Bcc</td>
						<td class="column-left">
							<input class="<?php if(!empty($this->errors['bcc'])) echo 'error'; ?>" type="text" value="<?php echo $this->var['bcc']; ?>" name="bcc">
						</td>
					</tr>
					<tr class="<?php if(empty($this->var['reply-to'])) echo 'hide'; ?>"> 
						<td class="border-left col-remove">
							<span class="remove" id="button-replyto" data-icon="&#xe093;"></span>
						</td>
						<td class="column-right" colspan="1">Reply-To</td>
						<td class="column-left">
							<input class="<?php if(!empty($this->errors['reply-to'])) echo 'error'; ?>" type="text" value="<?php echo $this->var['reply-to']; ?>" name="reply-to">
						</td>
					</tr>
					<tr class="<?php if(!empty($this->var['cc']) && !empty($this->var['bcc']) && !empty($this->var['reply-to'])) echo 'hide'; ?>"> 
						<td class="border-left column-left" colspan="3">
							<a id="add-cc" class="add <?php if(!empty($this->var['cc'])) echo ' hide'; ?>" href="javascript:void(0);"><span data-icon="&#xe004;"></span>Add Cc</a>
							<a id="add-bcc" class="add <?php if(!empty($this->var['bcc'])) echo ' hide'; ?>" href="javascript:void(0);"><span data-icon="&#xe004;"></span>Add Bcc</a>
							<a id="add-replyto" class="add <?php if(!empty($this->var['reply-to'])) echo ' hide'; ?>" href="javascript:void(0);"><span data-icon="&#xe004;"></span>Add Reply-To</a>
						</td>
					</tr>
					<tr>
						<td class="border-left column-right">Subject</td>
						<td class="column-left" colspan="2">
							<input class="<?php if(!empty($this->errors['subject'])) echo 'error'; ?>" type="text" value="<?php echo $this->var['subject']; ?>" name="subject">
						</td>
					</tr>	
					<tr>
	...
	private function _mceContent(){
		if(empty($this->var['message'])){
			$content = self::_mcePrintEntry();
		}
		else{
			$content = stripcslashes(html_entity_decode($this->var['message']));
		}
	...
	private function _mcePrintEntry(){
  	...
  	$html = "<span style='font-size: 12pt;'><strong> Entry Details:</strong></span>";
  	$html .= "<table style='width: 60%;' cellspacing='0' cellpadding='8' >";
		foreach($fields as $key => $value){
			$html .= "<tr>";
			if($key != 'guiform_date_submitted') $html .= "<td style='width: 30%;'>$value[0]</td><td>{{$key}}</td>";
			$html .= "</tr>";
		}
		$html .= "</table>";
	...
	function single_row( $item ) {
		...
		static $row_class = 'class="row"';
		$row_class = ( $row_class == 'class="row"' ? ' class="alternate row"' : 'class="row"' );
