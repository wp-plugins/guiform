...
class GuiForm_Render_Forms_Settings extends GuiForm_Render_Forms {
	...
	private function _save(){
		...
		if(isset($_POST['smtp_enable'])){
			if(empty($smtpHost)){
				$this->errors['smtp_host'] =  __('SMTP Host is required.');
			}
			
			if(empty($smtpPort)){
				$this->errors['smtp_port'] =  __('SMTP Port is required.');
			}
			else if(!preg_match('/^\d+$/', $smtpPort)){
				$this->errors['smtp_port'] =  __('SMTP Port is invalid.');
			}
		}
		...
		$this->_data = $guiform->form($this->form);
		
	}

	public function toHTML() {
		?>
		<div id="guiform-manager" class="wrap form-settings metabox-holder">
			<?php $this->navigation->toHTML(); ?>
			<?php GuiForm_Render_Forms::header('settings'); ?>
			<?php self::_toHTML(); ?>
		</div>
		<?php
	}
