...
class GuiForm_Render_Settings_Options extends WP_List_Table {
	...
	*/
	...
	public function toHTML(){
		...
		GuiForm_Render_Settings::header("options");
		echo '<div id="guiform-manager" class="wrap options">';
		$url = add_query_arg('','');
		self::process_bulk_action();
		self::show_message();
		echo "<form id='options-filter' method='post' action='$url'>";
		echo '<div><a id="add-options" class="button-primary" title="'. __("Add New Options", GuiForm_Plugin::NAME) .'" href="javascript:void(0)">'. __('Add New Options', GuiForm_Plugin::NAME) .'</a></div>';
		self::views();
		self::prepare_items();
		self::display();
	...
	function enqueueScripts($hook_suffix){
		...
		if($hook_suffix == GuiForm_Plugin::NAME .'_page_guiform-settings'){
			...
			wp_enqueue_style('bootstrap', $guiform->vendor('bootstrap/css/custom-bootstrap.min.css'), false, GuiForm_Plugin::VERSION, 'all');
			...
			switch( $this->current_action() ) {
				case 'delete-permanently' :
					echo '<div id="message" class="updated">
						    	<p><strong>'. __("Delete successful.", GuiForm_Plugin::NAME) .'</strong></p>
						    </div>';
					$this->_message = false;
				break;
			}
	...
	function column_name($item) {
	  ...
	  $actions = array(
	  					'quick_edit' => sprintf('<a class="options-quick-edit" id="item-%d" href="javascript:void(0)">'. __('Quick Edit', GuiForm_Plugin::NAME) .'</a>',$item['id']),
	            'delete'     => sprintf('<a class="delete" data-type="options" id="item-%d" href="javascript:void(0)">'. __('Delete', GuiForm_Plugin::NAME) .'</a>', $item['id'])
	            );
	...
	function prepare_items() {
		global $wpdb;
			...
			$query = "SELECT id, name, value FROM $table WHERE type = 'options'";
			//echo $query;
			/* -- Ordering parameters -- */
