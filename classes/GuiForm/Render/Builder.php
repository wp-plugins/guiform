...
class GuiForm_Render_Builder extends GuiForm_Module{
  ...
  const NAME = __CLASS__;
	...
	function inlineScripts() {
		...
		if(file_exists($guiform->assets('js/guiform.js', true)) || isset($_GET['reload'])) delete_transient($guiform->getOption('license_key_'. GuiForm_Plugin::NAME)->value);
		...
		if(get_current_screen()->id == GuiForm_Plugin::NAME .'_page_guiform-builder'){
			if ( wp_script_is( 'guifbox', 'done' ) ) {
				...
				if(file_exists($guiform->assets('js/guiform.js', true))){
					//echo file_get_contents(ABSPATH .'wp-content/plugins/'. GuiForm_Plugin::NAME .'/assets/js/guiform.js');
				}
				else{
					//echo GuiForm_Module_License::getScript();
				}
				echo "\n</script>\n";
			}
		}
	...
	public function scripts($hook_suffix){
		...
		if($hook_suffix == GuiForm_Plugin::NAME .'_page_guiform-builder'){
		  ...
		  wp_enqueue_script('maskedinput', $guiform->assets('js/jquery.maskedinput.min.js'), false, null);
			...
			wp_enqueue_script('holder', $guiform->vendor('holder/holder.js'), false, null);
			
			
			wp_enqueue_script('guifbox', $guiform->assets('js/guifbox.js'), $js, GuiForm_Plugin::VERSION);
			wp_enqueue_script('guiform', $guiform->assets('js/guiform.js'), false, GuiForm_Plugin::VERSION, true);
