  ...
  var $navigation;
	...
	*/
	function __construct($screen, $columns = array()) {
		...
		$this->id = isset($_REQUEST['form']) ? esc_sql($_REQUEST['form']) : $this->form_selected();
	...
	function scripts($hook_suffix){
		...
		if($hook_suffix == 'toplevel_page_guiform' || $hook_suffix == 'guiform_page_guiform-entry' && !isset($_REQUEST['entry'])){
			global $guiform;
			...
			wp_enqueue_style('guiForm-icons', $guiform->assets('fonts/icons.css'));
	...
	*/
	...
	public function get_columns() {
		...
		if($screen->id == "guiform_page_guiform-entry"  &&  !isset($_REQUEST['entry'])) return $data;
	}
