...
class GuiForm_Ajax{  
  ...
  private function action(){  
  	$this->action = esc_sql($_POST['action']);
		...
		add_action("wp_ajax_nopriv_$this->action", array($this, $callback));
  ...
  private function save_entries(){
  	...
  	if(isset($_POST['guiform-upload'])){
	  	...
	  	if(sizeof($guif_fields) == 0) die(json_encode(array('status' => 'fail', 'message' => "Unable to process you form!")));
