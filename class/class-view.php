...
class View{
	...
	function __construct($screen){
		...
		$this->form     = $guif->guiform($_GET['form'], 'row');
		$this->field    = $guif->entry($_GET['form'], $this->entry);
				...
				if($guif->is_serial($value)){
					...
					foreach($value as $file => $row){
						if($labels[$key][1] == "f_file"){
							$file = $guif->upload_folder($file, $row['path'], true);
							$html .= "<li><a target='_blank' href='$file'>".$row['name']." <span class='size'>".$guif->bytesToSize($row['size']) ."</span></a></li>";
						}
						else{
							$html .= "<li>$row</li>";
						}
	...
	function metabox_header(){
		...
		$preview = $guif->permalink($_REQUEST['form'], false, false); 
