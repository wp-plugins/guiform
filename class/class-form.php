...
class Form_List_Table extends WP_List_Table {
	...
	*/
	...
	function column_form_title($item) {
		global $guif;
		$entries_url = add_query_arg(array('page'=>'guiform_entries', 'form-list'=> $item->id));
		
	  $actions = array(
	  					'edit'      => sprintf('<a href="?page=guiform/form-builder&form=%d">Edit</a>',$item->id),
	            'view'      => sprintf('<a target="_blank" href="'. $guif->permalink() .'%d">Preview</a>',$item->id),
	...
	// Get entries search terms
	  ...
	  if(!empty($search)){
		...
		$query = "SELECT id, title, status, last_update, created FROM $wpdb->guiform ". $search_query;
