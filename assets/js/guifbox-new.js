...
(function( $, undefined ) {
    ...
    _build: function() {
        var bgHeader;
      	...
      	html = "<div id='guifbox'>\
				...
				(this.options.status != 'loading') ? $('#guifbox-header').append("<span data-icon='&#xe082;' id='guifbox-close'></span>") : $('#guifbox-close').remove();		
				...
				$('#guifbox-body').html(this.options.body).css({'overflow-x' : 'hidden', 'overflow-y' : 'auto'});
				this.options.maxScreen ? $('#guifbox-body').css({'max-height' : 'auto', 'overflow-x' : 'hidden', 'overflow-y' : 'auto'}) : $('#guifbox-body').css({'max-height' : this.options.maxHeight});
				
				$('#guifbox-window').css({'width': this.options.width});
				...
				$.each(options.tabs, function(key,value){
				    ($.isFunction(options.tabs[key])) ? alert(options.tabs[key]()) : alert(options.tabs[key]);
				});
    },
    ...
    _event: function() {
    	...
    	$('#guifbox-overlay, #guifbox-close, #guifbox-cancel').click(function(event){
				method._close();
			});
			
			$('#guifbox').keydown(function(event){
    ...
    _close : function(){
    	...
    	if(options.status == 'success' && options.reload == true){
				...
				$('#guifbox-body').html('<p>Page Loading</p>');
				options.reload = true
    	...
    	var method = this;
			$.ajax({
				...
				beforeSend: function(){
					...
					$('#guifbox-title').html('Loading');
					$('#guifbox-button, #guifbox-close').remove();
					$('#guifbox-body').html('<p>Please wait while the server processing your request.</p>');
