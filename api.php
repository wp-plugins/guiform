...
class GuiForm_API{
	 ...
	 * @return $url string
	 */
	public function upload_folder($file = "", $link = false){
		if(empty($file)){
			$day = date('j').'/';
		}
		else{
		...
		$url = ($link) ? get_site_url() .'/'. $this->get_option(0, 'upload_folder') : $this->upload_folder;
		return $url .'/'. date('Y') .'/'. date('n') .'/'. $day;
	}
	...
	public function guiform($id, $type = ""){
		...
		if(empty($type)){
  		return $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->guiform WHERE id = %d", $id), OBJECT_K);
  	}
  	if($type == "row"){
  		return $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->guiform WHERE id = %d", $id), ARRAY_A);
		}
  	else if($type == "data"){
  ...
  /**
	...
	public function mailer($id, $type = 'mail', $init = array()){
		...
		if($type == 'mail'){
			...
			if($init['subject'] == 'response'){
				
			}
			else if($init['subject'] == 'notification'){
				
			}
		
			$AddAddress = $init['mailto'];
			$subject = $init['subject'];
			$MsgHTML = $init['message'];
		  ...
		  $row = unserialize($data->value);
			...
			$html = "<strong>Greetings!</strong><br /><br />
							This is a test email generated by the GuiForm plugin.<br /><br />";
		...
		if(!$phpmailer->Send()) {
			...
			if($type !== 'mail'){
				echo __("Message sent! Please check your email for message.", 'guiform');
			}
		}
	}
